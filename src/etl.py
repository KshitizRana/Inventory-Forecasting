...
from utils import convert_timestamp_to_hourly, get_data

# Aggregate data
sales_df = convert_timestamp_to_hourly(get_data('data/sales.csv'),'timestamp')
...
sales_agg_merged.to_csv('data/sales_processed.csv',index=False)


stocklevel_df = convert_timestamp_to_hourly(get_data('data/sensor_stock_levels.csv'),'timestamp')
...
stocklevel_df.to_csv('data/stock_processed.csv')

stocktemp_df = convert_timestamp_to_hourly(get_data('data/sensor_storage_temperature.csv'),'timestamp')
stocktemp_df = stocktemp_df.groupby(by=['timestamp']).agg({'temperature' : 'mean'}).reset_index()
stocktemp_df.to_csv('data/temp_processed.csv',index=False)
